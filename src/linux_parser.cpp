...
vector<std::string> LinuxParser::GetAvailableDevices() {
    std::vector<std::string> devices;

    std::ifstream filestream("/proc/partitions");
    if (filestream.is_open()) {
        std::string line;
        while (std::getline(filestream, line)) {
            std::istringstream linestream(line);
            std::string major, minor, blocks, device;
            linestream >> major >> minor >> blocks >> device;

            // Exclude devices with minor number 0 (e.g., whole disks)
            if (minor != "0") {
...
vector<std::string> LinuxParser::GetAvailablePartitions() {
  std::vector<std::string> partitions;

  // Read the /proc/partitions file
  ...
  if (file.is_open()) {
    ...
    while (std::getline(file, line)) {
      ...
      iss >> major >> minor >> blocks >> partition;

      partitions.push_back("/dev/" + partition);
    }
    file.close();
  }
...
// Reads and returns Write Rate of given PID
double LinuxParser::GetDiskWriteRateForPID(int pid) {
    ...
    if (file.is_open()) {
        while (std::getline(file, line)) {
            if (line.find("write_bytes:") != std::string::npos) {
                ...
                unsigned long long writeBytes = std::stoull(valueStr);
                writeRate = static_cast<double>(writeBytes) / 1024.0;  // Convert to kilobytes
                break;
            }
        }
        file.close();
...
//Reads and returs Read Rate of given PID
double LinuxParser::GetDiskReadRateForPID(int pid) {
    std::string filename = "/proc/" + std::to_string(pid) + "/io";
    std::ifstream file(filename);
    std::string line;
    double readRate = 0.0;
    ...
    if (file.is_open()) {
        while (std::getline(file, line)) {
            if (line.find("read_bytes:") != std::string::npos) {
                std::size_t pos = line.find(":") + 1;
                std::string valueStr = line.substr(pos);
                unsigned long long readBytes = std::stoull(valueStr);
                readRate = static_cast<double>(readBytes) / 1024.0;  // Convert to kilobytes
                break;
            }
        }
        file.close();
